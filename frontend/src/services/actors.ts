...
export const fetchActor = async (id): Promise<Actor> => {
    const res = await getActor(id);
    return res.data;
}

export const createActor = async (data): Promise<Actor> => {
    const res = await createActors(data);
    return res.data;
}

export const updateActor = async (id, data): Promise<Actor> => {
...
export const useUpdateActor = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: Actor }) =>
      updateActor(id, data),
